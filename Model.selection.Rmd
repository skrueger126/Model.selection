...
The goal of this first study was to investigate mound site preference of leaf cutter ants (*Atta cephalotes*) in Belize. A total of 6 mounds were surveyed and 6 randomized control sites were selected that did not have any mounds. The data includes one response variable: presence, and four predictor variables: distance to forest edge (m), sun exposure (%), elevation (ft), and soil temperature (degrees F). 
...
Sun exposure (%) and soil temperature looked closely related, which is what we would expect, but I chose to ignore that and continue with fitting four candidate models that explain variation in mound presence. (Note: Since the response variable was binary, I had to run a generalized linear model)

```{r, message=FALSE, warning=FALSE}
mod1 <- glm(presence ~ Elevation + distance.to.edge + sun.exposure , family = binomial(link = "logit"), data = ants)
mod2 <- glm(presence ~ soil.temp + distance.to.edge, family = binomial(link = "logit"), data = ants)
mod3 <- glm(presence ~ Elevation + distance.to.edge, family = binomial(link = "logit"), data = ants)
mod4 <- glm(presence ~ sun.exposure, family = binomial(link = "logit"), data = ants)
```
...
Looks like model 4 containing an intercept (Int), and sun exposure (sun.exp) is best model with a weight of 0.749. It is 0.749/0.120 = 6.24 times more likely to be the best explanation (hypothesis) for variation in mound presence. 
...
We can see that this function has narrowed the model selection down to two. 
...
Looking at the output above, there is plenty of evidence for *sun exposure* (weight is close to one), but much less for *soil temperature*. 
...
p1 <- ggplot(ants, aes(sun.exposure, presence)) + 
  geom_point() +
  xlab("Sun Exposure (%)") +
  ylab("Mound Presence") +
...
We can see that this function has narrowed the models down to two. 
...
```{r}
p2 <- ggplot(bats, aes(winter.duration, survival)) + 
  geom_point() +
